...
class soundplayer:
    ...
    def playfixedmasker(self, name):
        ...
        sd.play(fixedmaskers*fixedmastergain, fs)
        sd.wait()
    def playfixedmaskereval(self, name):
        ...
        print('now playing fixed masker {} with gain: {} as DOA {}'.format(fixedmasker, fixedmastergain, self.currentdoa))
        while True:
            sd.play(fixedmaskers, fs, loop=True)
            if is_key_pressed():
                break
    def play4maskers(self):
        ...
        if self.msgdict != None: #If there is a prediction
            ...
            if (self.msgdict['predictions'][self.maskerindex1]["id"] != self.currentmasker) or (self.msgdict['predictions'][self.maskerindex2]["id"] != self.currentmasker) or (self.msgdict['predictions'][self.maskerindex3]["id"] != self.currentmasker) or (self.msgdict['predictions'][self.maskerindex4]["id"] != self.currentmasker) or (abs(self.msgdict['predictions'][self.maskercounter]["gain"]-self.maskergain)*self.gainweight>self.maskerdiff) or (abs(self.currentdoa - self.msgdict["doa"])>self.doadiff):
                ...
                predictionlist = [self.currentmasker1, self.maskergain1, self.currentmasker2, self.maskergain2, self.currentmasker3, self.maskergain3, self.currentmasker4, self.maskergain4]
                appendlist = ['AMSS4', predictionlist]         
        ...
        try:
            with sf.SoundFile(self.maskerpath + (self.currentmasker1 if not MEOW else "meow") +'.wav') as f1:
                with sf.SoundFile(self.maskerpath + (self.currentmasker2 if not MEOW else "meow") +'.wav') as f2:
                    with sf.SoundFile(self.maskerpath + (self.currentmasker3 if not MEOW else "meow") +'.wav') as f3:
                        with sf.SoundFile(self.maskerpath + (self.currentmasker4 if not MEOW else "meow") +'.wav') as f4:
                            print("stream created using {} with gain: {} at DOA: {}".format(self.currentmasker,self.weightedgain,self.currentdoa))
                            stream = sd.OutputStream(
                                samplerate=f1.samplerate, blocksize=self.blocksize,
                                device=7, channels=4,
                                callback=self.streamcallback, finished_callback=self.event.set)
                            with stream:
                                timeout = self.blocksize * self.buffersize / f1.samplerate * 1000
                                for _ in range(int(self.buffersize)):
                                    data1 = self.spatialize(f1.read(self.blocksize, always_2d=True)*self.weightedgain1, self.currentdoa)
                                    data2 = self.spatialize(f2.read(self.blocksize, always_2d=True)*self.weightedgain2, self.currentdoa)
                                    data3 = self.spatialize(f3.read(self.blocksize, always_2d=True)*self.weightedgain2, self.currentdoa)
                                    data4 = self.spatialize(f4.read(self.blocksize, always_2d=True)*self.weightedgain2, self.currentdoa)
                                    data15 = np.add(data1, data2)
                                    data25 = np.add(data3, data4)
                                    data = np.add(data15, data25) * 0.5
                                    if not len(data):
                                        break
    ...
    def play2maskers(self):
        ...
        if self.msgdict != None: #If there is a prediction
            ...
            if (self.msgdict['predictions'][self.maskerindex1]["id"] != self.currentmasker) or (self.msgdict['predictions'][self.maskerindex2]["id"] != self.currentmasker) or (abs(self.msgdict['predictions'][self.maskercounter]["gain"]-self.maskergain)*self.gainweight>self.maskerdiff) or (abs(self.currentdoa - self.msgdict["doa"])>self.doadiff):
                self.maskergain1 = self.msgdict['predictions'][self.maskerindex1]["gain"]
                self.maskergain2 = self.msgdict['predictions'][self.maskerindex2]["gain"]
                predictionlist= [self.currentmasker1, self.maskergain1, self.currentmasker2, self.maskergain2]
                appendlist = ['AMSS2', predictionlist]         
                predictiondf.loc[len(predictiondf)] = appendlist
                # if self.maskergain (set in previous step) less than gainlimit (set at 1000)
                if self.maskergain1*self.gainweight < self.gainlimit:
                    print("self.maskergain1 = {}".format(self.maskergain1))
    ...
    def soundlooper(self):
        ...
        for name in orderlist:
            ...
            if name == 'amss1':
                ...
                sd.play(nexttrack, nexttrackfs)
                sd.wait()
                self.playmasker()
                print(self.beforeevaluationmsg)
                sd.play(evaluation, evaluationfs)
                sd.wait()
