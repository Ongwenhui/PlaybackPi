...
#set alsaaudio
import alsaaudio
mixer = alsaaudio.Mixer('IEC958 In')
mixer.setvolume(100)
...
varymaskers = False
MEOW = False
#Change this for Holland Close Amplification
HCMULTIPLIER = 1
...
class soundplayer:
    def __init__(self):
        ...
        # self.msgdict = None
        self.currentmasker = "Sine_1kHz_0.25_30s" if not MEOW else "meow"
        self.maskerpath = "/home/pi/speakertestmaskers/"
        self.maskergain = 1
        self.gainweight = 1    
    ...
    def playmasker(self):
        ...
        if self.msgdict != None:
            ...
            if (self.msgdict['predictions'][self.maskercounter]["id"] != self.currentmasker) or (abs(self.msgdict['predictions'][self.maskercounter]["gain"]-self.maskergain)*self.gainweight>self.maskerdiff) or (abs(self.currentdoa - self.msgdict["doa"])>self.doadiff):
                ...
                if self.maskergain*self.gainweight < self.gainlimit:
                    ...
                    self.weightedgain = 1
                self.weightedgain = self.weightedgain
                print("self.weightedgain = {}, HCMULTIPLER is {}".format(self.weightedgain,HCMULTIPLIER))
                self.currentmasker = 'Sine_1khz_0.25_30s'
        ...
        try:
            ...
            with sf.SoundFile('/home/pi/speakertestmaskers/Sine_1khz_0.25_30s.wav') as f:
                        ...
                        if not self.q2.empty():
                            ...
                            if (self.msgdict['predictions'][0]["id"] != self.currentmaskerorig) or (abs(self.msgdict['predictions'][0]["gain"]-self.maskergainorig)*self.gainweight>self.maskerdiff) or (abs(self.currentdoa - self.msgdict["doa"])>self.doadiff):
                                ...
                                if newamssgain >45 and newamssgain <= 83:
                                    ...
                                    newamssgain = 46
                                newweightedgain = 1
                                newweightedgain = newweightedgain*HCMULTIPLIER
